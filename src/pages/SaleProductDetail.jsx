...
import useSaleProducts from "../hooks/SaleCard";
import "../NewArrivalProducts/NewArrivalProducts.css";

const SaleProductDetail = () => {
  const { id } = useParams();
  const product = useSaleProducts().find((item) => String(item.id) === id);
  ...
  if (!product) {
    return (
      <div className="detail-card">
        <h2>Sale product not found</h2>
        <Link to="/sale">Back to Sale</Link>
      </div>
    );
  }
  ...
  return (
    <div className="detail-card">
      <div className="detail-header">
        <h2>{product.title}</h2>
        <Link to="/sale" className="detail-close">
          Back to Sale
        </Link>
      </div>
